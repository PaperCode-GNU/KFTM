...
import numpy as np
import pickle as pk
from data_process.data_util import seg_sentence, get_cutter
from utils.log import set_logger

def format_string(s):
    return s.replace("b", "").replace("\t", " ").replace("t", "").replace("\r\n","")
...
def get_statistics_for_filter(train_path, valid_path, total_law, law2num, total_accu, accu2num):
    ...
    strpass = '二审'
    total_sample_train, total_sample_valid, frequency_law, frequency_accu = 0, 0, [0] * total_law, [0] * total_accu
    ...
    #         frequency_law[law2num[temp_law]] += 1
    #         frequency_accu[accu2num[temp_accu]] += 1
    #         total_sample_valid += 1

    print('After filtering, the total number of samples train is {0}'.format(total_sample_train))
    #print('After filtering, the total number of samples valid is {0}'.format(total_sample_valid))
...
def filter_law_and_accu(path, total_law, num2law, frequency_law, total_accu, num2accu, frequency_accu):
    ...
    law_file = open(path + 'law_accu/new_law_big.txt', 'w', encoding='utf-8')
    accu_file = open(path + 'law_accu/new_accu_big.txt', 'w', encoding='utf-8')

    for i in range(total_law):
        if frequency_law[i] >= 100:
        #if frequency_law[i] >= 2:
...
def filter_samples(path, train_path, valid_path, test_path, law2num, filter_law_list, filter_law2num, accu2num,
    ...
    filter_Rtrain = open(path + 'CTM/Rtrain_ctm.json', 'w', encoding='utf-8')
    filter_Rvalid = open(path + 'CTM/Rvalid_ctm.json', 'w', encoding='utf-8')
    filter_Rtest = open(path + 'CTM/Rtest_ctm.json', 'w', encoding='utf-8')

    Cutter = thulac.thulac(seg_only=True)   #分词器，默认使用thulac
    ...
    for line in Rtrain.readlines(): #训练数据
        dic = json.loads(line)
        unique_accu = list(set(dic["meta"]["accusation"]))
        unique_law = list(set(dic["meta"]["relevant_articles"]))
        if strpass in dic["fact"] or len(unique_accu) > 1 or len(unique_law) > 1:
            ...
            if law2num[temp_law] in filter_law_list and accu2num[temp_accu] in filter_accu_list:
                ...
                #fact_cut = Cutter.cut(fact.strip(), text=True) #分词
                sentence= seg_sentence(fact.strip(), cut)
                fact_cut = sentence
                ...
                #这里对后面的刑期做分类，默认为2（有期徒刑），1（死刑），0（无期徒刑），然后根据相应刑期的月份分为十大类
    ...
    for line in Rvalid.readlines():
        ...
        if strpass in dic["fact"] or len(unique_accu) > 1 or len(unique_law) > 1:
            ...
            if law2num[templaw] in filter_law_list and accu2num[tempaccu] in filter_accu_list: #这里就是再过滤低频样本
                total_valid += 1
                if dic["meta"]["term_of_imprisonment"]["imprisonment"] > longest:
                    longest = dic["meta"]["term_of_imprisonment"]["imprisonment"]
                # if dic["meta"]["term_of_imprisonment"]["death_penalty"] or \
